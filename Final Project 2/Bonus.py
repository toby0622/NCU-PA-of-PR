...
colors = plt.cm.rainbow(np.linspace(0, 1, len(set(y_test))))

for i, label in enumerate(set(y_test)):
    mask = y_test == label
    ax.scatter(X_test_lda[mask, 0], X_test_lda[mask, 1], X_test_lda[mask, 2],
               color=colors[i], label=f'Class {label}', s=60)

ax.set_xlabel('LDA Feature 1', fontsize=14)
ax.set_ylabel('LDA Feature 2', fontsize=14)
ax.set_zlabel('LDA Feature 3', fontsize=14)
ax.set_title('LDA Classification Results (3D)', fontsize=16)
